...
# clustering (K-means), PCA, and kriging maps.
#
# How to run (example):
...
option_list <- list(
  ...
  optparse::make_option("--epsg_projected", type = "integer", default = 9208,
                        help = "Projected CRS for kriging/maps (default: %default; VN-2000 / TM-3 104-45)"),
...
# -----------------------
# 2) Standards (QCVN) and variables
# -----------------------
std <- list(
  nitrite   = 0.05,
  ammonium  = 0.3,
...
p_elbow <- ggplot2::ggplot(elbow_df, ggplot2::aes(x = k, y = wss)) +
  ...
  ggplot2::geom_vline(xintercept = opt$k_opt, linetype = "dashed", linewidth = 0.8) +
  ggplot2::annotate(
...
loading_long <- loading_long %>%
  dplyr::mutate(
    param_label = dplyr::if_else(key %in% names(label_map), label_map[key], key)
  )
...
# -----------------------
# 7) Join coordinates to WQI table
# -----------------------
...
writexl::write_xlsx(dat_wqi2, file.path(opt$out_dir, "tables", "data_wqi_with_coordinates.xlsx"))
...
# -----------------------
# 8) Kriging maps (by round)
# -----------------------
message("7) Kriging mapping ...")
